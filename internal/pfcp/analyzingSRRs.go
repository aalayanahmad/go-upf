...
func GetSRRContent(srrID uint8) ([]*QoSControlInfo, error) {
	SRR_Mu.RLock()
	defer SRR_Mu.RUnlock()

	// Load the value from the sync.Map
	value, exists := SotredSrrsToBeUsedByUpf.Load(srrID)
	if !exists {
		return nil, fmt.Errorf("SRR ID %d not found", srrID)
	}

	// Attempt to assert the loaded value to the expected type
	srrInfos, ok := value.([]*QoSControlInfo)
	if !ok {
		return nil, fmt.Errorf("invalid type for SRR ID %d", srrID)
	}
	return srrInfos, nil
}
...
func GetQoSFlowMonitoringContent() error {
	...
	for _, srrInfo := range srrInfos {
		...
		if qfi == uint8(1) {
			qfi_destination = "10.100.200.12" //change according to destination1 IP
		}
		if qfi == uint8(2) {
			qfi_destination = "10.100.200.16" //change according to destination2 IP
		}
